...
public class StructureController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		toggleSidepanel.setOnMouseEntered(e->{
			...
			((Label) toggleSidepanel.getChildren().get(1)).setFont(Font.font(((Label) toggleSidepanel.getChildren().get(1)).getFont().getFamily(), FontWeight.BOLD, ((Label) toggleSidepanel.getChildren().get(1)).getFont().getSize()));
		});
		...
		toggleSidepanel.setOnMouseExited(e->{
			...
			((Label)toggleSidepanel.getChildren().get(1)).setFont(Font.font(((Label) toggleSidepanel.getChildren().get(1)).getFont().getFamily(), FontWeight.NORMAL, ((Label) toggleSidepanel.getChildren().get(1)).getFont().getSize()));
	...
	public static void addButton(String fxml,String name) {
		try {
			...
			((ImageView) button.getChildren().get(0)).setImage(new Image(new FileInputStream(new File("src/view/sideBarIcons/"+fxml+".png"))));
			button.setCursor(Cursor.HAND);
			button.setOnMouseEntered(e->{
				button.setStyle("-fx-background-color:#0D0F48;");
				((Label) button.getChildren().get(1)).setFont(Font.font(((Label) button.getChildren().get(1)).getFont().getFamily(), FontWeight.BOLD, ((Label) button.getChildren().get(1)).getFont().getSize()));
			});
			
			button.setOnMouseExited(e->{
				button.setStyle("-fx-background-color:transparent;");
				((Label) button.getChildren().get(1)).setFont(Font.font(((Label) button.getChildren().get(1)).getFont().getFamily(), FontWeight.NORMAL, ((Label) button.getChildren().get(1)).getFont().getSize()));
